...
class ModelTrainer:
    ...
    def cross_validate(self, X: pd.DataFrame, y: pd.Series, cv: int = CV_FOLDS) -> Dict[str, float]:
        ...
        cv_metrics = {"cv_mean": scores.mean(), "cv_std": scores.std(), "cv_scores": scores}
        logger.info(
            f"Cross-validation scores: {cv_metrics['cv_mean']:.4f} (+/- {cv_metrics['cv_std']:.4f})"
        )
        return cv_metrics

    def hyperparameter_tune(
        self, X_train: pd.DataFrame, y_train: pd.Series, param_grid: Dict, cv: int = CV_FOLDS
    ) -> None:
        """
